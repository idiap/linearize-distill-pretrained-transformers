...
def main(args):
    ...
    if rank == 0 and not args.no_write:
        ...
        kwargs = {"wandb": {'name': run_name, 'tags': ['distill']}}
    ...
    hp_ = copy.copy(hp)
    hp_.asr_use_mamba = False
    teacher_model = model.Model(hp_)

    if args.use_teacher_snapshots:
        all_teachers = checkpoint.find_all_ckpt(args.teacher_path)
        all_teachers = [(step, path) for step, path in all_teachers if step % hp.teacher_snapshot_cycle == 0]
        logging.info("Teacher snapshots: " + '\n'.join([str(t) for t in all_teachers]))
        all_teachers = dict(all_teachers)
    else:
        checkpoint.load_model(args.teacher_path, teacher_model)
        all_teachers = {}
    teacher_model.to(device)
    teacher_model.train()

    if not args.use_teacher_snapshots:
        logging.info("Initializing student model from teacher weights")
        m.load_state_dict(teacher_model.state_dict(), strict=False)
    else:
        logging.info("Initializing student model from original weights")
    ...
    if args.restore_from:
        global_step = checkpoint.load_model(args.restore_from, m, optim, sched, map_location,
                                            args.reset_training, args.load_no_opt)
    ...
    def find_current_teacher(teacher_paths, global_step):
        min_step = 1e9
        for step, path in teacher_paths.items():
            if step < min_step and step > global_step:
                min_step = step
        if min_step == 1e9:
            return teacher_paths[max(teacher_paths.keys())]
        return teacher_paths[min_step]

    if args.use_teacher_snapshots:
        teacher_path = find_current_teacher(all_teachers, global_step)
        logging.info("Load teacher model from %s" % teacher_path)
        checkpoint.load_model(teacher_path, teacher_model, map_location=map_location)

        batches = feeder_eval.fetch_data()
        batches = batches[:hp.max_eval_batches]
        with accelerator.autocast():
            metrics = infer_batches(teacher_model, batches, None, hp, device, processor, write_output=False)
            logging.info("Teacher model metrics: %s" % str(metrics))
            teacher_model.train()
    ...
    loss_windows = {}
    for key in ['ctc', 'layer', 'logit', 'classifier', 'total']:
        loss_windows[key] = infolog.ValueWindow(100)
    ...
    while True:
        ...
        accum_loss_keys = ['loss', 'ctc_loss', 'decoder_loss', 'classifier_loss', 'layer_loss', 'logit_loss']
        ...
        for i, batch in enumerate(accum_batches):
            ...
            oom = False
            try:
                if hp.alpha_layer_distill_loss > 0 or hp.alpha_logits_distill_loss > 0:
                    with torch.no_grad(), accelerator.autocast():
                        teacher_outputs = teacher_model(**batch)
                    teacher_outputs = dict_send_to(teacher_outputs, device=None, detach=True)
                else:
                    teacher_outputs = {}
                ...
                logging.error("Frames: %.2E, Frames^2: %.2E" % (batch['inputs'].shape[0] * batch['inputs'].shape[1],
                                           batch['inputs'].shape[0] * batch['inputs'].shape[1] * batch['inputs'].shape[1]))
                traceback.print_exc()
        ...
        if global_step in all_teachers:
            next_teacher_path = find_current_teacher(all_teachers, global_step)
            checkpoint.load_model(next_teacher_path, teacher_model, map_location=map_location)
            logging.info("Switch to teacher model at %s" % next_teacher_path)
        ...
        if rank == 0:
            ...
            for key in loss_windows:
                if key + '_loss' in losses:
                    loss_windows[key].append(losses[key + '_loss'])
            loss_windows['total'].append(losses['loss'])
            sample_window.append(losses['n_fin'])
            length_window.append(losses['n_frames'])
            loss_message = '; '.join(
                ['%s=%.4f, avg. %.4f' % (k, losses[k + '_loss'], loss_windows[k].average) for k in loss_windows if k + '_loss' in losses])
            message = (
                        '[Step %d] %.3f sec/step (%.3f), lr=%.04E, loss=%.4f (%s), %d (%.2f) samples, %d (%.2f) frames, grad_norm=%.3f'
                        % (global_step, dur, time_window.average, sched.get_last_lr()[-1],
                           loss_windows['total'].average, loss_message, losses['n_fin'], sample_window.average,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--load_no_opt", action='store_true', default=False)
    ...
    parser.add_argument('--use_teacher_snapshots', action='store_true')
    parser.add_argument('--teacher_snapshot_cycle', type=int, default=1)
