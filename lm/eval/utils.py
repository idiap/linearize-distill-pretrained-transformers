...
def generate_completions(model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, add_special_tokens=True, disable_tqdm=False, **generation_kwargs):
    ...
    is_mamba = getattr(model.config, 'use_mamba', False) or 'mamba' in model.name_or_path.lower()
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        try:
            batch_outputs = model.generate(
                ...
                attention_mask=attention_mask if not is_mamba else None,
...
def score_completions(model, tokenizer, scoring_examples, batch_size=1, aggregation="sum", disable_tqdm=False):
    ...
    unrolled_examples = []
    for idx, scoring_example in enumerate(scoring_examples):
    ...
    # roll up the scores
    rolled_up_scores = [dict() for _ in range(len(scoring_examples))]
    for unrolled_example, score in zip(unrolled_examples, scores):
        completion = unrolled_example["completion"]
        rolled_up_scores[unrolled_example["idx"]][completion] = score
...
def load_hf_lm(
    ...
    if os.path.exists(os.path.join(model_name_or_path, 'config.json')):
        saved_config = json.load(open(os.path.join(model_name_or_path, 'config.json')))
        if 'HybridMamba' in saved_config['architectures'][0]:
            if 'gpt_neox_config' in saved_config:
                base_model = saved_config['gpt_neox_config']['_name_or_path']
                use_config = False
        else:
            base_model = saved_config.get('_name_or_path', model_name_or_path)
    model = AutoModelForCausalLM.from_pretrained(
        base_model,
        config=config if use_config else None,
    )
    if getattr(config, 'use_mamba', False):
        if getattr(config, 'use_mamba2', False):
    ...
    if model_ckpt is None and os.path.exists(os.path.join(model_name_or_path, 'pytorch_model.bin')):
        model_ckpt = os.path.join(model_name_or_path, 'pytorch_model.bin')
    if model_ckpt is not None:
        model_ckpt = torch.load(model_ckpt)
    if os.path.exists(os.path.join(model_name_or_path, 'peft_config', 'adapter_config.json')):
        teacher_config = PeftConfig.from_pretrained(os.path.join(model_name_or_path, 'peft_config'))
        model = get_peft_model(model, teacher_config)
        if model_ckpt is not None:
            model_ckpt = {('base_model.model.' + k if not k.startswith('base_model.model') else k): v for k, v in model_ckpt.items()}
        model.load_state_dict(model_ckpt, strict=False)
        model = model.merge_and_unload()
    elif model_ckpt is not None:
        model.load_state_dict(model_ckpt, strict=False)
    ...
    try:
        ...
        print(f"Failed to convert model to BetterTransformer. Error: {e}")
    model.eval()
    return model
