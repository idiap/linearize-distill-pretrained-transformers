...
def main():
    ...
    teacher_ckpt_paths = list(glob.glob(args.teacher_ckpt_pattern))
    if all([os.path.split(p)[-1].split('_')[-1].isdigit() for p in teacher_ckpt_paths]):
        teacher_ckpt_paths = sorted(teacher_ckpt_paths, key=lambda x: int(os.path.split(x)[-1].split('_')[-1]))
    else:
        teacher_ckpt_paths = sorted(teacher_ckpt_paths)
    teacher_ckpt_paths = [os.path.join(t, 'pytorch_model.bin') if os.path.isdir(t) else t for t in teacher_ckpt_paths]
    ...
    if args.with_tracking:
        ...
        if args.run_name:
            ...
            kwargs["wandb"]["tags"] = ["joint_ft_snapshot_distill"]
    ...
    teacher_update_cycle = args.max_train_steps // len(teacher_ckpt_paths)

    teacher_update_at = [(i * teacher_update_cycle, teacher_ckpt_paths[i]) for i in range(len(teacher_ckpt_paths))]
    logger.info("Teacher update at: " + "\n".join([str(i) for i in teacher_update_at]))
    teacher_update_at = dict(teacher_update_at)
    def update_teacher(step):
        teacher_model.load_state_dict(torch.load(teacher_update_at[step], map_location=accelerator.device))
        logger.info(f"Teacher updated to {teacher_update_at[step]}")

        eval_metric = do_eval(teacher_model)
        logger.info(f"Teacher eval:  {eval_metric}")
        if args.with_tracking:
            accelerator.log(
                {
                    "teacher_accuracy" if args.task_name is not None else "teacher_glue": eval_metric,
                },
                step=step,
            )
    update_teacher(0)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.with_tracking:
            total_loss = 0
        for step, batch in enumerate(train_dataloader): # We should also resume the teacher; haven't done that as we didn't use resuming in our experiments
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                ...
                if completed_steps in teacher_update_at:
                    update_teacher(completed_steps)
