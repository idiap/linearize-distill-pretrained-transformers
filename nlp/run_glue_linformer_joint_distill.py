...
def parse_args():
    ...
    parser.add_argument("--save_teacher_checkpoint", action="store_true")
    parser.add_argument("--teacher_checkpoint_dir", type=str, default=None)
    ...
    parser.add_argument("--teacher_update_cycle", type=int, default=1)
    ...
    parser.add_argument("--early_stop_bar", type=float, default=None)
...
def main():
    ...
    teacher_optimizer_grouped_params = [
        {
    ...
    if args.teacher_lr is None:
        args.teacher_lr = args.lr
    teacher_optimizer = torch.optim.AdamW(teacher_optimizer_grouped_params, lr=args.teacher_lr)
    ...
    if args.with_tracking:
        ...
        if args.run_name:
            ...
            kwargs["wandb"]["tags"] = ["joint_ft_distill"]
    ...
    teacher_loss_window = ValueWindow(10)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            if args.resume_from_checkpoint and epoch == starting_epoch:
                if resume_step is not None and step < resume_step:
                    completed_steps += 1
                    continue
            teacher_outputs_with_hidden = teacher_model(**batch, output_hidden_states=True)
            teacher_loss = teacher_outputs_with_hidden.loss
            teacher_loss_window.append(teacher_loss.item())
            teacher_loss = teacher_loss / args.gradient_accumulation_steps
            if ((step // args.gradient_accumulation_steps) + 1) % args.teacher_update_cycle == 0:
                teacher_loss.backward()

            if (step + 1) % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                if ((step + 1) // args.gradient_accumulation_steps) % args.teacher_update_cycle == 0:
                    teacher_optimizer.step()
                    teacher_lr_scheduler.step()
                    teacher_completed_steps += 1
                    if teacher_completed_steps % args.eval_interval == 0:
                        eval_metric = do_eval(teacher_model)
                        logger.info(f"Teacher step {teacher_completed_steps} (stu. {completed_steps}):  {eval_metric}")
                        if args.with_tracking:
                            accelerator.log(
                                {
                                    "teacher_accuracy" if args.task_name is not None else "glue": eval_metric,
                                },
                                step=completed_steps,
                            )
                    if teacher_completed_steps % 10 == 0 and args.with_tracking:
                        accelerator.log(
                            {
                                "teacher_loss": teacher_loss_window.average,
                                "teacher_learning_rate": teacher_optimizer.param_groups[0]["lr"],
                                "teacher_step": teacher_completed_steps,
                            },
                            step=completed_steps,
                        )
            ...
            loss = 0
            ce_loss_window.append(outputs_with_hidden.loss.item())
            ...
            if alpha_layer_distill_loss != 0:
                for i in range(13):
                    layer_distill_loss += loss_mse(outputs_with_hidden[2][i],
                                                   teacher_outputs_with_hidden[2][i].detach())
            ...
            progress_bar.desc = (f"loss: {loss_window.average:.4f} ({ce_loss_window.average:.4f} "
                                 f"{distill_loss_window.average:.4f}) (tc: {teacher_loss_window.average:4f}) "
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                ...
                if completed_steps % args.eval_interval == 0 or step == len(train_dataloader) - 1:
                    ...
                    if args.early_stop_bar is not None and list(eval_metric.values())[0] < args.early_stop_bar:
                        if args.early_stop_steps is not None and completed_steps >= args.early_stop_steps:
                            logger.info(
                                f"Early stopping at step {completed_steps} with metric {list(eval_metric.values())[0]}")
                            stopped = True
                            break
            ...
            if isinstance(checkpointing_steps, int):
                if completed_steps % checkpointing_steps == 0 and completed_steps > 0:
                    ...
                    if args.save_teacher_checkpoint:
                        teacher_output_dir = f"step_{teacher_completed_steps}"
                        if args.teacher_checkpoint_dir is not None:
                            teacher_output_dir = os.path.join(args.teacher_checkpoint_dir, teacher_output_dir)
                            os.makedirs(teacher_output_dir, exist_ok=True)
                            torch.save(teacher_model.state_dict(), os.path.join(teacher_output_dir, "pytorch_model.bin"))
        ...
        if args.checkpointing_steps == "epoch":
            ...
            if args.output_dir is not None and args.save_checkpoint:
                ...
                accelerator.save_state(output_dir, safe_serialization=False)
        if args.save_teacher_checkpoint:
            teacher_output_dir = f"epoch_{epoch}"
            if args.teacher_checkpoint_dir is not None:
                teacher_output_dir = os.path.join(args.teacher_checkpoint_dir, teacher_output_dir)
                os.makedirs(teacher_output_dir, exist_ok=True)
                torch.save(teacher_model.state_dict(), os.path.join(teacher_output_dir, "pytorch_model.bin"))
